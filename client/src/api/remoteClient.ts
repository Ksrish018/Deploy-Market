// The original server-backed implementation — used whenever this app is
// actually served by a real backend (dev, the .exe, Docker/Fly.io, Vercel).
// See client.ts for the dispatcher that picks this or localClient.ts.
...
export const BriefApi = {
  ...
  downloadXlsx: (cycleId: string, briefId: string) => {
    window.location.assign(`/api/cycles/${cycleId}/brief/${briefId}/export.xlsx`);
  },
  downloadMarkdown: (cycleId: string, briefId: string) => {
    window.location.assign(`/api/cycles/${cycleId}/brief/${briefId}/export.md`);
  },
  downloadRunLogXlsx: (cycleId: string) => {
    window.location.assign(`/api/cycles/${cycleId}/run-logs/export.xlsx`);
  },
};
