// Ported from server/src/graph.js.
import { isCutoffPassed } from './helpers';
import type { AppState } from './seed';
import type { Cycle, GraphNode, GraphResponse } from '../types';

export function buildGraph(state: AppState, cycle: Cycle): GraphResponse {
  ...
  const brief = [...state.briefs].filter((b) => b.cycleId === cycle.id).sort((a, b) => new Date(b.generatedAt).getTime() - new Date(a.generatedAt).getTime())[0];

  const blocked = isCutoffPassed(cycle);
  const gateState: GraphNode['state'] = blocked ? 'blocked' : (cycle.state === 'collecting' ? 'running' : 'done');
